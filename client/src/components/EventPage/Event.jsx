      ...
      <div className="events-grid">
        {loading
          ...
          : currentEvents.map((event) => {
            ...
            let isClosed = false;
            let noSeats = event.availableSeats === 0;
            ...
            if (foundMonth !== -1 && foundDay !== -1) {
              ...
              start = new Date(`${foundYear}-${mStr}-${dStr}T${sH}:${sM}:00+05:30`);
              ...
              isEnded = (now >= end) || (event.status === "completed");
              isBookingActive = now <= (windowEnd.getTime() - 2 * 60 * 1000) && !isEnded;
              isClosed = now > windowEnd && !isEnded;

              buttonLabel = isEnded ? "Event Ended" :
            ...
            return (
              <div className={`event-card ${isEnded ? "event-ended-card" : ""} ${noSeats && !isEnded ? "event-soldout-card" : ""}`} key={event._id}>
                <div className="event-image-container">
                  {event.isUpcoming && (
                    <span className="upcoming-badge">Upcoming</span>
                  )}
                  {isEnded && <div className="event-status-overlay ended">EVENT ENDED</div>}
                  {!isEnded && noSeats && <div className="event-status-overlay sold-out">SOLD OUT</div>}
